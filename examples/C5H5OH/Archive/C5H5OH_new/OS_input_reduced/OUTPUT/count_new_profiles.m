...
for i = 1:size(List_of_Exp,1)
    ...
    if str2double(A{1,3}) > 1
    else
        m = i
        fprintf('The dataset has only one species')
    end
    
    if strcmp(A{1,1},'Batch')
    else
        fprintf("A type of reactor keyword is different from Batch")
    end

    if strcmp(A{1,2},'m_SP')
    else
        fprintf("A type of target treatment is different from m_SP")
    end
...
fprintf(['The removed ones are ' num2str(100-tot_profiles*100/(341*7)) ' percent of the total initial \n' ]);