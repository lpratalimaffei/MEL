...
class INPUT_MESS:
       ...
       def data_names(self):
              ...
              species_names_bimolB = np.array([],dtype='<U16')
              look_for_species = 0
              look_for_bimol_fragment = 0
              with open(self.cwd + '/me_ktp.inp') as myfile:
                     for line in myfile:
                            ...
                            if line.find('PressureList') != -1:
                                   ...
                                   #print(pressures)
                                   #print(len(pressures))
                            elif line.find('TemperatureList') != -1:
                                   temperatures = [x.strip() for x in line.split()]
                                   del temperatures[0]
                            elif (line.find('Well ')) != -1 and look_for_species == 1:
                                   full_line = [x.strip() for x in line.split()]
                                   species_names_unimol = np.append(species_names_unimol,full_line[1])
                                   species_names_bimolB = np.append(species_names_bimolB,'')

                            elif (line.find('Bimolecular ')) != -1 and look_for_species == 1:
                                   look_for_bimol_fragment = 1
                                   full_line = [x.strip() for x in line.split()]
                                   species_names_bimol = np.append(species_names_bimol,full_line[1])
                            if (line.find('Fragment')) != -1 and look_for_bimol_fragment > 0:
                                   look_for_bimol_fragment += 1
                                   if look_for_bimol_fragment == 3:
                                          full_line = [x.strip() for x in line.split()]
                                          species_names_bimolB = np.append(species_names_bimolB,full_line[1])
                                          look_for_bimol_fragment = 0
              ...
              self.P_LIST = np.array(pressures,dtype=np.float32)
              self.T_LIST = np.array(temperatures,dtype=np.int16)
              
              self.species_names = np.append(species_names_unimol,species_names_bimol)
              print(species_names_unimol,species_names_bimol,species_names_bimolB)
              return self.P_LIST,self.T_LIST,self.species_names,species_names_bimolB
       ...
       def MATRIX(self):
              ...
              # len works on b
              check_P = len(self.T_LIST)*len(self.species_names)*len(self.P_LIST)
              # define current checks
              check_P_curr = 0
              check_list = 0
              with open(self.cwd + '/rate.out') as myfile:
                     for line in myfile:
                            ...
                            if ((check_list == 1) and (check_P_curr < check_P)):
                                   if any(line.find(T) != -1 for T in np.array(self.T_LIST,dtype=str)) and (line.find('Pressure') == -1):  # add the check on 'Pressure' in case the values of temperature and pressure are accidentally the same
                                          #print(line)
                                          check_P_curr += 1
              ...
              #print(self.matrix)
              self.matrix_float = np.array(self.matrix,dtype=np.float64)
              
              # remove negative values from the matrix
              n_T = len(self.T_LIST)
              n_P = len(self.P_LIST)
              warnings_neg = '' # generate list of warnings for negative values
              for ii,row in enumerate(self.matrix_float):
                     ...
                     for mask_neg_i in mask_neg[0]:
                            ...
                            warnings_neg = warnings_neg + 'removed negative k_{R} at {T} K and {P} atm, be careful \n'.format(R=self.species_names[R],T=self.T_LIST[T],P=self.P_LIST[P])


              print(warnings_neg)
              np.savetxt('warnings_negval.txt',[warnings_neg],fmt='%s')  

              self.n_T = len(self.T_LIST)
              self.n_P = len(self.P_LIST)

              return self.matrix_float


       def MATRIX_TP(self,T,P):
              """
              ...
              P_index = np.where((P==P_LIST))[0][0]  # index [0][0] to save just the index itself
              T_index = np.where((T==T_LIST))[0][0]
              #print(self.matrix_float)
              for ii in range(0,n_species):
                     # identify the string to place in the row: ki->prods
                     rates_row = self.matrix_float[ii*(self.n_P)*(self.n_T)+P_index*(self.n_T)+T_index,:]
       ...
       def REAC_P(self,P,reac):
              ...
              if not isinstance(reac,str):
              ...
              elif sum(np.array([reac == r_list for r_list in self.species_names])) == 0:
                     ...
                     P_index = np.where((P==self.P_LIST))[0][0]
                     reac_index = np.array([reac == r_list for r_list in self.species_names],dtype=int)
                     ii_reac = np.where(reac_index==1)[0][0] # index of the reactant
                     ii_in = ii_reac*(self.n_P)*(self.n_T)+P_index*(self.n_T)
                     rates_reac = self.matrix_float[ii_in:ii_in+self.n_T,:]
...
class INPUT_CHEMKIN:
       ...
       """
       EXTRACT THE ARRHENIUS PARAMETERS OF ALL THE RATE CONSTANTS YOU FIND IN A CKI FILE
       """
       def __init__(self,cwd):
              self.cwd = cwd
       ...
       def data_names(self):
              ...
              species_names = np.array([],dtype='<U16')          # names of all the primary species
              species_names_bimol = np.array([],dtype='<U16')    # names of the "second" reactant found in bimolecular reaction channels
              check_reactions = 0         # check in the file if you reading the part with all the reactions

              with open(self.cwd + '/kin.CKI') as myfile:
                     for line in myfile:
                            ...
                            if check_reactions == 1 and line.find('=>') != -1 and line.strip()[0] != '!':
                                   ...
                                   for x in reacting_species:
                                          if np.array([x[0] == SP for SP in species_names]).any() != True:
                                                 ...
                                                 if len(x) == 1:
                                                        species_names_bimol = np.append(species_names_bimol,'')
                                                 elif len(x) == 2:
                                                        species_names_bimol = np.append(species_names_bimol,x[1])
                                                 else:
                                                        raise ValueError('Wrong number of elements when reading the reaction: line ' + line)
              self.species_names = species_names
              self.species_names_bimol = species_names_bimol
       ...
       def copy_CKI_processed(self,newpath,PRODSINKS,ISOM_EQUIL,REAC,PRODS):
              ...
              for idx,row in enumerate(newfile):
                     ...
                     if ISOM_EQUIL == 1 and row.find('=>') != -1 and row.strip()[0] != '!':
                            ...
                            product = [x.strip() for x in product.split('+')][0]  # only bimolecular products will be meaningful
                            ...
                            # compare the reactant and the product with the bimolecular species
                            # 1. create a mask for bimolecular species indices
                            #mask_bimol = np.where(self.species_names_bimol != '')
                            #species_names_bimol_first = self.species_names[mask_bimol]
                            #print(species_names_bimol_first)
                            # 2. compare the name of the reactant or the product with the bimolecular species and delete the bimolecular reactions
                            #if np.array([reactant==np.array(species_names_bimol_first)]).any() or np.array([product==np.array(species_names_bimol_first)]).any():
                            #       delete = 'YES'
                            #else:
                            #       delete = 'NO'
