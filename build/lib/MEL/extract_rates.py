...
def MATRIX(cwd, P_LIST, T_LIST, species_names, del_allT_whenneg = False):
    ...
    """
    print('del_allT_whenneg', del_allT_whenneg)
    # pre-allocation
    matrix_list = []
    capture_list = []
                ...
                if len(rates_TP_iiprod[rates_TP_iiprod == 0]) > 0.15*len(T_LIST):
                    ...
                    warnings_neg = warnings_neg + \
                        'neg vals in k({}->{}) at {} atm set to 0\n'.format(
                            R, Pr, P)
    """ as above but delete full set of rate constants (all P)
    for ii_reac, R in enumerate(species_names):
        prods = species_names[species_names != R]
        ii_in = ii_reac*(n_P)*(n_T)
        for ii_prod, Pr in enumerate(prods):
            rates_TP_iiprod = matrix_float[ii_in:ii_in+n_T*n_P, ii_prod]
            if len(rates_TP_iiprod[rates_TP_iiprod == 0]) > 0.2*n_T*n_P:
                if del_allT_whenneg:
                    #delete all pressures
                    matrix_float[ii_in:ii_in+n_T*n_P, ii_prod] = 0
                warnings_neg = warnings_neg + \
                    'neg/too high vals in k({}->{}) at set to 0\n'.format(
                        R, Pr,)
    """                
