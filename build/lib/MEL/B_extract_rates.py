...
def readmechanism(input_type,cwd):
       ...
       if input_type == 'MESS':
              # extract parameters
              P_LIST,T_LIST,species_names,species_names_bimol_frag2 = data_names_mess(os.path.join(cwd,'inp'))
              # extract matrix of rate constants
              rates = MATRIX(os.path.join(cwd,'inp'),P_LIST,T_LIST,species_names)
              mech_dict = dict(zip(['P_VECT_MESS','T_VECT_MESS','SPECIES','SPECIES_BIMOL','rates'],[P_LIST,T_LIST,species_names,species_names_bimol_frag2,rates]))
       ...
       with open(os.path.join(cwd,'me_ktp.inp')) as myfile:
              for line in myfile:
                     ...
                     if line.find('PressureList') != -1:
                            ...
                            species_names_bimol_frag2 = np.append(species_names_bimol_frag2,'')
                     ...
                     if (line.find('Fragment')) != -1 and look_for_bimol_fragment > 0:
                            ...
                            if look_for_bimol_fragment == 3:
                                   full_line = [x.strip() for x in line.split()]
                                   species_names_bimol_frag2 = np.append(species_names_bimol_frag2,full_line[1])
                                   look_for_bimol_fragment = 0
       ...
       with open(cwd + '/rate.out') as myfile:
              for line in myfile:
                     ...
                     if ((check_list == 1) and (check_P_curr < check_P)):
                            if any(line.find(T) != -1 for T in np.array(T_LIST,dtype=str)) and (line.find('Pressure') == -1):  # add the check on 'Pressure' in case the values of temperature and pressure are accidentally the same
                                   #print(line)
                                   check_P_curr += 1
...
def REAC_P(P,reac,P_LIST,T_LIST,species_names,matrix_float):
       ...
       if not isinstance(reac,str):
              ...
              reac_index = np.array([reac == r_list for r_list in species_names],dtype=int)
