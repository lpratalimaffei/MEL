...
class PLOTTING:
       ...
       def plot_data(self,profiles_Pi,REAC,PRODS,SPECIES,P):
              ...
              self.palette_series = pd.Series(colorsrgb[:len(SPECIES)],index=SPECIES)
       ...
       def plot_exp_profiles(self,N_INIT):
              ...
              for T in self.T_VECT:
                     ...
                     # set vect for maxt
                     maxt = []
                     # set axis: only col_plot if 1 row
                     try:
                     ...
                     for key in list(self.profiles.keys()):
                            ...
                            for Si in self.SPECIES:
                                   if np.where(tW_DF.columns==Si)[0].size > 0:                                                 
                                          axx.plot(t,tW_DF[Si],color=self.palette_series[Si],ls=self.lstyles[kk])
                     ...
                     try:
                            ...
                            axes[col_plot].set_xlabel(r'$t$ [s]',fontsize=self.fsize_vect[1])
                     ...
                     axx.set_title((str(T) + ' K'),fontsize=self.fsize_vect[2],fontweight='bold')
                     axx.set_xlim([0,min(maxt)])
                     axx.set_ylim([0,N_INIT])
                     axx.axes.ticklabel_format(axis='both', style='sci', scilimits=(0,0),useLocale=True,useMathText=True)
       ...
       def plot_data_reac(self,profiles_Pi,REAC,P,N_INIT_REAC):
              ...
              for T in self.T_VECT:
                     ...
                     axx.set_title((str(T) + ' K'),fontsize=self.fsize_vect[2],fontweight='bold')
                     axx.set_ylim([0,np.max(np.max(tW_DF[REAC]))])
                     axx.axes.ticklabel_format(axis='both', style='sci', scilimits=(0,0),useLocale=True,useMathText=True)
                     axx.yaxis.offsetText.set_fontsize(self.fsize_vect[0])
                     axx.xaxis.set_tick_params(labelsize=self.fsize_vect[0])
                     axx.xaxis.offsetText.set_fontsize(self.fsize_vect[0])
