...
class PLOTTING:
    ...
    def plot_data(self, profiles_Pi, REAC, PRODS, SPECIES, P):
        ...
        if int(len(self.T_VECT)**0.5) == (len(self.T_VECT)**0.5):
            ...
            self.n_rows = int(len(self.T_VECT)**0.5) + \
                int((len(self.T_VECT)/self.n_cols) > int(len(self.T_VECT)**0.5))
    ...
    def plot_exp_profiles(self, N_INIT):
        ...
        for T in self.T_VECT:
            ...
            if T == self.T_VECT[-1] and col_plot != self.n_cols-1:
                for miss_col in np.arange(col_plot+1, self.n_cols):
                    axes[self.n_rows-2,
                         miss_col].set_xlabel(r'$t$ [s]', fontsize=self.fsize_vect[1])
        ...
        # save the figure
        fig_path = (self.path_plots + '/Prof_EXP_' +
                    self.REAC + '_' + str(self.P) + '_atm.png')
    ...
    def plot_data_reac(self, profiles_Pi, REAC, P, N_INIT_REAC):
        ...
        fig.tight_layout(pad=0.1, w_pad=0.1)
        # save the figure
        fig_path = (os.path.join(self.path_plots, 'Prof_EXP_' +
                                 self.REAC + 'composition_' + str(self.P) + '_atm.png'))
        if os.path.isfile(fig_path):
            os.remove(fig_path)
        fig.savefig(fig_path, dpi=200)
