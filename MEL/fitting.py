    ...
    def fits_lumped_k(self, SPECIES_BIMOL_SERIES, bfthreshold = 0.):
        ...
        for Pr_i in self.PRODS:
            ...
            if len(index_ovf) == 0:
                ...
                if R2adj < 0.6:
                    comments_MAT[0, i_prod] += ' ! too LOW R2; commented '
            else:
   ...
   # at this point, call the class WRITE_MECH_CKI from the C_preprocessing module and write the new mech
    ...
    def WRITE_PLOG_FITS(self, FITS_DICT, P_VECT, cwd):
        ...
        for reac in reactions:
            ...
            listoflows = ['LOW' in comm for comm in comms]
            if (all(listinfsnans) or all(listoflows)):
                flag = '!'
            ...
            if len(P_VECT) > 1:
                for P in P_VECT:
                    ...
                    if flag == '' and any((par == 'nan' or par == 'inf' or 'LOW R2' in par) for par in DF_reac.loc[Pi][['k0', 'alpha', 'EA', 'comments']]):
