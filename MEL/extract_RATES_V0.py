    ...
    for P in P_VECT:
        ...
        if input_type == 'MESS':
            ...
            if isinstance(REAC,np.ndarray):
                ...
                for rr in REAC:
                    kR_j_rr = extr_rates.REAC_P(P,rr,P_VECT_MESS,T_VECT_MESS,SPECIES,rates)
                    flag_rr,Err_rr,T_VECT_rr = preproc.CHECK_REACTIVITY(kR_j_rr,T_VECT,T_VECT_MESS)
        ...
        if isinstance(REAC,np.ndarray):
            try:
                BR_L_REAC,T_VECT = preproc.BRANCHING_LUMPEDREAC(os.path.join(cwd,jobtype),REACLUMPED,REAC,T_VECT,T_VECT_rr_len,P,P_VECT)
